...
class Jet2(pygame.sprite.Sprite):
    def __init__(self, x, y, ally_x, ally_y):
        super().__init__()
        self.image = pygame.image.load('./assets/plane2.png').convert()
        self.image_og_size = self.image.get_size()  # Resizing base, returns a tuple
        self.image_new_size = (self.image_og_size[0] * .25, self.image_og_size[1] * .25)
